...
export default function BusinessProfileView() {
  ...
  const [bizType, setBizType] = useState('Food Vendor');
  const [regNum, setRegNum] = useState('N/A');
  const [bizAddress, setBizAddress] = useState('N/A');
  const [bizDesc, setBizDesc] = useState('N/A');
    ...
    const fetchBizData = async () => {
      try {
        ...
        if (statusRes.success && statusRes.data) {
          setRegNum('REG-' + Math.floor(Math.random() * 90000 + 10000));
          setBizType(statusRes.data.businessType || 'Food Vendor');
        }

        const shopRes = await businessService.getMyFoodShop();
        if (shopRes.success && shopRes.data) {
          setBizAddress(shopRes.data.address || 'N/A');
          setBizDesc(shopRes.data.description || 'N/A');
        }
      } catch (e) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: themeColors.background }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.profileCard, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
          <Pressable onPress={handleAvatarPress} style={styles.avatarWrapper}>
            {profile?.profilePictureUrl ? (
              ...
              <View style={[styles.avatarPlaceholder, { backgroundColor: 'rgba(46, 158, 191, 0.1)' }]}>
                <StoreIcon size={40} color={Colors.brand.accent} />
              </View>
            )}
      ...
      <View style={styles.content}>
        ...
        <Text style={[styles.sectionTitle, { color: themeColors.textSecondary }]}>Owner Details</Text>
        <Card variant="elevated" style={styles.sectionCard} padding="medium">
          
          <View style={styles.fieldItem}>
            <View style={styles.fieldHeader}>
              <UserIcon size={16} color={Colors.brand.accent} style={{ marginRight: 10 }} />
              <Text style={[styles.fieldLabel, { color: themeColors.textSecondary }]}>Owner Name</Text>
            </View>
            {isEditing ? (
              <TextInput
          ...
          <View style={styles.fieldItem}>
            <View style={styles.fieldHeader}>
              <MailIcon size={16} color={Colors.brand.accent} style={{ marginRight: 10 }} />
              <Text style={[styles.fieldLabel, { color: themeColors.textSecondary }]}>Business Email</Text>
            </View>
          ...
          <View style={styles.fieldItem}>
            <View style={styles.fieldHeader}>
              <PhoneIcon size={16} color={Colors.brand.accent} style={{ marginRight: 10 }} />
              <Text style={[styles.fieldLabel, { color: themeColors.textSecondary }]}>Business Phone</Text>
            </View>
            {isEditing ? (
              <TextInput
        ...
        {/* ── Section: Shop Metadata ── */}
        <Text style={[styles.sectionTitle, { color: themeColors.textSecondary }]}>Shop Credentials</Text>
        <Card variant="elevated" style={styles.sectionCard} padding="medium">
          
          <View style={styles.fieldItem}>
            <View style={styles.fieldHeader}>
              <StoreIcon size={16} color={Colors.brand.accent} style={{ marginRight: 10 }} />
              <Text style={[styles.fieldLabel, { color: themeColors.textSecondary }]}>Business Category</Text>
            </View>
            <Text style={[styles.fieldValue, { color: themeColors.text }]}>{bizType}</Text>
          </View>
          ...
          <View style={styles.fieldItem}>
            <View style={styles.fieldHeader}>
              <DocIcon size={16} color={Colors.brand.accent} style={{ marginRight: 10 }} />
              <Text style={[styles.fieldLabel, { color: themeColors.textSecondary }]}>Registration Number</Text>
            </View>
            <Text style={[styles.fieldValue, { color: themeColors.text }]}>{regNum}</Text>
          </View>
          ...
          <View style={styles.fieldItem}>
            <View style={styles.fieldHeader}>
              <MapPinIcon size={16} color={Colors.brand.accent} style={{ marginRight: 10 }} />
              <Text style={[styles.fieldLabel, { color: themeColors.textSecondary }]}>Shop Location</Text>
            </View>
            <Text style={[styles.fieldValue, { color: themeColors.text }]} numberOfLines={2}>{bizAddress}</Text>
          </View>
          ...
          <View style={styles.fieldItem}>
            <View style={styles.fieldHeader}>
              <FileText size={16} color={Colors.brand.accent} style={{ marginRight: 10 }} />
              <Text style={[styles.fieldLabel, { color: themeColors.textSecondary }]}>Description</Text>
            </View>
            <Text style={[styles.fieldValue, { color: themeColors.text }]} numberOfLines={3}>{bizDesc}</Text>
          </View>
        </Card>

        {/* ── Section: Settings ── */}
        <Text style={[styles.sectionTitle, { color: themeColors.textSecondary }]}>Business Settings</Text>
        ...
        <View style={styles.footerButtons}>
          ...
          >
            ...
            <Text style={[styles.deleteLinkText, { color: themeColors.danger }]}>Delete Business account permanently</Text>
          </Pressable>
        </View>
      ...
      >
        <View style={styles.modalContent}>
          ...
          <Button
            ...
            variant="primary"
            style={{ marginTop: 20, backgroundColor: Colors.brand.accent }}
      ...
      >
        <View style={styles.modalContent}>
          <Text style={[styles.warningText, { color: themeColors.danger }]}>
            WARNING: This action is irreversible. All listed menu items, deals, and registration files will be deleted from the server.
          </Text>
          <TextInput
